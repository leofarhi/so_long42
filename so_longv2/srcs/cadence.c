...
static int lerpf(float a, float b, float t)
{
	return (a + t * (b - a));
}

static t_bool	near_center(t_game *game)
{
	int i;
...
void	cadence_draw(t_game *game)
{
	...
	if (ncenter)
		size = (t_vector2){game->assets.ui[1]->size.x/2, game->assets.ui[1]->size.y/2};
	else
	{
		size = (t_vector2){game->assets.ui[1]->size.x/3, game->assets.ui[1]->size.y/3};
		if (game->sprite_update)
			perfect = FALSE;
	}
	mlxe_draw_subtexture_size(game->window, game->assets.ui[1],
		(t_coords){(t_rect){0, 0,
		game->assets.ui[1]->size.x, game->assets.ui[1]->size.y}, (t_rect){
		(game->window->buffer->size.x/2) - size.x/2,
		game->window->buffer->size.y - size.y - 10,
		size.x, size.y}}, 0xFFFFFFFF);
	if (ncenter && game->cadence.player_moving)
		perfect = TRUE;
	if (perfect)
	{
		...
		(VEC2){(game->window->buffer->size.x/2) - 50, game->window->buffer->size.y - 50});
	}
}
