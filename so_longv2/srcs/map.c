	...
	while (i < game->window->buffer->size.y)
	{
		...
		while (j < game->window->buffer->size.x)
		{
			mlxe_draw_subtexture(game->window,
				...
				(t_vector2){j, i},
				(t_rect){(TILE_SIZE/2), 3 * (TILE_SIZE/2), TILE_SIZE, TILE_SIZE});
			j += TILE_SIZE;
		}
		i += TILE_SIZE;
	}
...
void camera_goto(t_game *game, t_vector2 pos, int smoothness)
{
	if (smoothness < 1)
		smoothness = 1;
	game->map.cam.x += (pos.x - game->map.cam.x - game->window->buffer->size.x / 2) / smoothness;
	game->map.cam.y += (pos.y - game->map.cam.y - game->window->buffer->size.y / 2) / smoothness;
}
