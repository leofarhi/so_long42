...
void	exit_loop(t_window *window, void *data)
{
	...
	if (game->sprite_update)
	{
		...
		if (i > 60)
		{
			game->exit->sprite_idx = 1;
			game->window->funct_ptr = main_loop;
		}
	}
	draw_map(game);
	mlxe_draw_sprite(game->window, game->exit->sprites[1],
...
void	quit_loop(t_window *window, void *data)
{
	...
	{
		ft_putstr_fd("Congratulations! You finished the game in ", 1);
	...
	draw_text_outline(window, game->assets.font,
		"Congratulations!", (t_vector2){200, 250});
	mlxe_render(window);
}

void	exit_update(t_game *game, t_entity *entity)
{
	if (game->map.collectible_countdown == 0
		&& game->player->pos.x == entity->pos.x
		&& game->player->pos.y == entity->pos.y)
	{
